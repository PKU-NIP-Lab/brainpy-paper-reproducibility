...
if run_on == 'cpp_standalone':
  ...
  # set_device("cython", directory='brian2_cython')
  pass
...
# sAMPA, x, sNMDA, sGABA are synaptic conductance stored pre-synaptically
...
class Simulation(object):
  ...
  def run(self, T, randseed=None):
    ...
    self.model.reinit()
    self.network.run(T)
    return device._last_run_time

  def savespikes(self, filename_exc, filename_inh):
    print("Saving excitatory spike times to " + filename_exc)
    np.savetxt(filename_exc,
               list(zip(list(self.model.mons['spikesE'].i),
                        list(self.model.mons['spikesE'].t))),
               fmt='%-9d %25.18e',
               header='{:<8} {:<25}'.format('Neuron', 'Time (s)'))

    print("Saving inhibitory spike times to " + filename_inh)
    np.savetxt(filename_inh,
               list(zip(list(self.model.mons['spikesI'].i),
                        list(self.model.mons['spikesI'].t))),
               fmt='%-9d %25.18e',
               header='{:<8} {:<25}'.format('Neuron', 'Time (s)'))
...
def simulate(scale):
  ...
  model_params = dict(
    ...
    gAMPA_E=80 / scale * nS,
    gNMDA_E=264 / scale * nS,
    gGABA_E=520 / scale * nS,

    # Unscaled recurrent synaptic conductances (onto inhibitory)
    gAMPA_I=64 / scale * nS,
    gNMDA_I=208 / scale * nS,
    gGABA_I=400 / scale * nS,
  ...
  sim = Simulation(model_params, stim_params, sim_dt, T)
  t0 = time.time()
  t = sim.run(T, randseed=4)
  t1 = time.time()

  num = model_params['N_E'] + model_params['N_I']

  print(f'Network size {num}, simulation time {t1 - t0} s, device time {device._last_run_time} s')
  ...
  t_run = device._last_run_time
  return num, t_run, t1 - t0
...
def benchmark():
  ...
  with open(f'speed_results/brian2-DecisionMaking-{run_on}.json', 'w') as fout:
